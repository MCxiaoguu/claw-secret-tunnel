#!/usr/bin/env bash
# Install + load the One-Time Secret Tunnel plugin (@mcxiaoguu/secret-tunnel) on an
# OpenClaw gateway: clone/build from source, link-install, enable, restart the gateway
# via whatever supervisor it uses, and verify it loaded.
#
# Run ON the gateway:
...
echo "## [1/4] detect how the gateway runs"
...
echo "## [2/4] install plugin (clone + build + link + enable)"
...
echo "## [3/4] restart gateway (WeChat drops for a few seconds)"
...
echo "## [4/4] verify the plugin loaded"
